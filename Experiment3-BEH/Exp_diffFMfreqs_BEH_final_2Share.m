...
for subj = 1:length(subjects)
    ...
    for currFM = 1:length(FM)    
        ...
        [groupData(subj).ratebyPhase(currFM,:), ~, ~, ~, groupData(subj).RT(currFM,:), groupData(subj).FalsA(currFM,:)] = get_hitRatebyPhase1500ms_1Cycle(Stim(fmCode(currFM,:)), responseMain(fmCode(currFM,:)), phaseBins);
...
%exclude participants with mean HR <0.2 & >0.9, because best phase can not
...
[p1,z1]=circ_rtest(Pphase(:,1));
[p2,z2]=circ_rtest(Pphase(:,2));
[p3,z3]=circ_rtest(Pphase(:,3));
[p4,z4]=circ_rtest(Pphase(:,4));

%load (fullfile(mainDIR,'GroupData_diffFM_1500ms'))
%Parametric Hotelling paired sample test for equal angular means
for var1 =1:4
    for var2 = 1:4
 [pval(var1,var2), F(var1,var2)] = circ_htest(Pphase(:,var1),Pphase(:,var2));
    end
end
...
for freq = 1:4
    for st = 1:size(groupData(subj).FalsA,2)
        if ~isempty(groupData(subj).FalsA(freq,st).Time)
         FA(subj,freq) = FA(subj,freq)+ length(groupData(subj).FalsA(freq,st).Time);
        end
...
%GAPSIZE
[PFA,TABLEFA,STATSFA] = friedman(FA);

%posthoc
[FAstats.P(1), FAstats.FAH(1), FAstats.s(1).FASTATS] = signrank(FA(:,1),FA(:,2),'method','approximate');
[FAstats.P(2), FAstats.FAH(2), FAstats.s(2).FASTATS] = signrank(FA(:,1),FA(:,3),'method','approximate');
[FAstats.P(3), FAstats.FAH(3), FAstats.s(3).FASTATS] = signrank(FA(:,1),FA(:,4),'method','approximate');
[FAstats.P(4), FAstats.FAH(4), FAstats.s(4).FASTATS] = signrank(FA(:,2),FA(:,3),'method','approximate');
[FAstats.P(5), FAstats.FAH(5), FAstats.s(5).FASTATS] = signrank(FA(:,2),FA(:,4),'method','approximate');
[FAstats.P(6), FAstats.FAH(6), FAstats.s(6).FASTATS] = signrank(FA(:,3),FA(:,4),'method','approximate');
for pit=1:length(FAstats.P)
FAstats.bonfHolms(pit)=FAstats.P(pit).*(sum(FAstats.P>FAstats.P(pit))+1);
end
FAstats.bonfP = FAstats.P*length(FAstats.P);

FATable = table([FA(:,1); FA(:,2); FA(:,3); FA(:,4)], [ones(length(FA(:,1)),1)-.2; ones(length(FA(:,1)),1)+1; ones(length(FA(:,1)),1)+2.2; ones(length(FA(:,1)),1)+3.4],'VariableNames',...
    {'FA','Freq'});
FAlm = fitlm(FATable,'FA~Freq');
figure, plot(FAlm)

mean(FA,'all')


save (fullfile(mainDIR,'GroupData_diffFM_1500ms_final','-v7.3'))
